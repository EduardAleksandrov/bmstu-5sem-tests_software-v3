...
bool loadSavedGame(GameState& state, std::set<std::pair<int, int>>& player1Shots, std::set<std::pair<int, int>>& player2Shots) {
    ...
    state.player1Shots.clear();
    state.player2Shots.clear();
    if (!file.is_open()) {
    ...
    for (int i = 0; i < SIZE; ++i) {
        std::string line;
        if (std::getline(file, line)) {
            state.player1Shots.push_back(std::vector<char>(line.begin(), line.end()));
        }
    }
    for (int i = 0; i < SIZE; ++i) {
        std::string line;
        if (std::getline(file, line)) {
            state.player2Shots.push_back(std::vector<char>(line.begin(), line.end()));
        }
    }
