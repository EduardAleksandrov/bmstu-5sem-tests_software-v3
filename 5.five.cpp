/*
    Игра морской бой
    Проверка количества кораблей работает.
    Авторизация
*/
#include <iostream>
#include <fstream>
#include <vector>
#include <string>
#include <sstream>

#include "5.excepts.hpp"

const int SIZE = 10;
const char EMPTY = '0';
const char SHIP = '1';
const char HIT = 'X';
const char MISS = 'O';

struct GameState {
    std::vector<std::vector<char>> player1Board;
    std::vector<std::vector<char>> player2Board;
    std::vector<std::vector<char>> player1Shots; // добавлены, чтобы не затирались
    std::vector<std::vector<char>> player2Shots;
    int currentPlayer;
    int turnCount;
};
const int FOUR_DECK_SHIP = 1;
const int THREE_DECK_SHIP = 2;
const int TWO_DECK_SHIP = 3;
const int ONE_DECK_SHIP = 4;


bool checkShipCounts(const std::vector<std::vector<char>>& board) {
    int fourDeckCount = 0;
    int threeDeckCount = 0;
    int twoDeckCount = 0;
    int oneDeckCount = 0;

    std::vector<std::vector<bool>> visited(SIZE, std::vector<bool>(SIZE, false));

    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            if (board[i][j] == SHIP && !visited[i][j]) {
                // Проверка горизонтально
                if (j + 3 < SIZE && board[i][j + 1] == SHIP && board[i][j + 2] == SHIP && board[i][j + 3] == SHIP) {
                    fourDeckCount++;
                    visited[i][j] = true;
                    visited[i][j + 1] = true;
                    visited[i][j + 2] = true;
                    visited[i][j + 3] = true;
                    j += 3; // Пропустить проверенные клетки
                    continue;
                } else if (j + 2 < SIZE && board[i][j + 1] == SHIP && board[i][j + 2] == SHIP) {
                    threeDeckCount++;
                    visited[i][j] = true;
                    visited[i][j + 1] = true;
                    visited[i][j + 2] = true;
                    j += 2; // Пропустить проверенные клетки
                    continue;
                } else if (j + 1 < SIZE && board[i][j + 1] == SHIP) {
                    twoDeckCount++;
                    visited[i][j] = true;
                    visited[i][j + 1] = true;
                    j += 1; // Пропустить проверенные клетки
                    continue;
                } 
                    

                // Проверка вертикально
                if (i + 3 < SIZE && board[i + 1][j] == SHIP && board[i + 2][j] == SHIP && board[i + 3][j] == SHIP) {
                    fourDeckCount++;
                    visited[i][j] = true;
                    visited[i + 1][j] = true;
                    visited[i + 2][j] = true;
                    visited[i + 3][j] = true;
                    continue;
                } else if (i + 2 < SIZE && board[i + 1][j] == SHIP && board[i + 2][j] == SHIP) {
                    threeDeckCount++;
                    visited[i][j] = true;
                    visited[i + 1][j] = true;
                    visited[i + 2][j] = true;
                    continue;
                } else if (i + 1 < SIZE && board[i + 1][j] == SHIP) {
                    twoDeckCount++;
                    visited[i][j] = true;
                    visited[i + 1][j] = true;
                    continue;
                }

                oneDeckCount++;
                visited[i][j] = true; // Отметить как посещенное
                continue;
            }
        }
    }
    std::cout << fourDeckCount << " " << threeDeckCount <<" " << twoDeckCount <<" " << oneDeckCount << std::endl;

    // Проверка на соответствие количеству кораблей
    return (fourDeckCount == 1 && threeDeckCount == 2 && twoDeckCount == 3 && oneDeckCount == 4);
}


void loadBoard(const std::string& filename, std::vector<std::vector<char>>& board) {
    std::ifstream file(filename);
    if (file.is_open()) {
        board.clear(); // Очистка доски перед загрузкой
        for (int i = 0; i < SIZE; ++i) {
            std::string line;
            if (std::getline(file, line)) {
                board.push_back(std::vector<char>(line.begin(), line.end()));
            } else {
                std::cerr << "Error reading line " << i + 1 << " from " << filename << std::endl;
                break; // Прерывание, если не удалось прочитать строку
            }
        }
        file.close();
    } else {
        std::cerr << "Could not open file " << filename << std::endl;
    }
}

void saveGame(const GameState& state) {
    std::ofstream file1("lastgame_player_1.cfg", std::ios::out);
    std::ofstream file2("lastgame_player_2.cfg", std::ios::out);
    std::ofstream file3("lastgame.cfg", std::ios::out);
    std::ofstream file4("lastgame_shots_player_1.cfg", std::ios::out);
    std::ofstream file5("lastgame_shots_player_2.cfg", std::ios::out);

    for (const auto& row : state.player1Board) {
        for (char cell : row) {
            file1 << cell;
        }
        file1 << std::endl;
    }

    for (const auto& row : state.player2Board) {
        for (char cell : row) {
            file2 << cell;
        }
        file2 << std::endl;
    }

    file3 << state.turnCount << std::endl;
    file3 << state.currentPlayer << std::endl;

    for (const auto& row : state.player1Shots) {
        for (char cell : row) {
            file4 << cell;
        }
        file4 << std::endl;
    }

    for (const auto& row : state.player2Shots) {
        for (char cell : row) {
            file5 << cell;
        }
        file5 << std::endl;
    }

    file1.close();
    file2.close();
    file3.close();
    file4.close();
    file5.close();
}

void printBoard(const std::vector<std::vector<char>>& board, const std::string& title) {
    std::cout << title << std::endl;
    std::cout << "  a b c d e f g h i j" << std::endl;
    for (int i = 0; i < SIZE; ++i) {
        std::cout << i + 1 << " ";
        for (char cell : board[i]) {
            std::cout << cell << " ";
        }
        std::cout << std::endl;
    }
}

bool isValidInput(const std::string& input) {
    return (input.length() == 2 && input[0] >= 'a' && input[0] <= 'j' && input[1] >= '1' && input[1] <= '9') ||
           (input == "exit");
}

void processTurn(GameState& state) {
    std::string input;
    int x, y;
    while (true) {
        std::cout << "Player " << state.currentPlayer << ", enter your shot (e.g., a1) or 'exit': ";
        std::cin >> input;

        if (input == "exit") {
            saveGame(state);
            std::cout << "Game saved. Exiting..." << std::endl;
            return;
        }

        if (!isValidInput(input)) {
            std::cout << "Invalid input. Please try again." << std::endl;
            continue;
        }

        x = input[1] - '1';
        y = input[0] - 'a';

        state.turnCount++; // количество шагов

        if (state.currentPlayer == 1) {
            if (state.player2Shots[x][y] != EMPTY) {
                std::cout << "You have already shot here. Try again." << std::endl;
                continue;
            }

            if (state.player2Board[x][y] == SHIP) {
                state.player2Shots[x][y] = HIT;
                std::cout << "Hit!" << std::endl;
            } else {
                state.player2Shots[x][y] = MISS;
                std::cout << "Miss!" << std::endl;
                state.currentPlayer = 2; // Переход хода к игроку 2
            }
        } else {
            if (state.player1Shots[x][y] != EMPTY) {
                std::cout << "You have already shot here. Try again." << std::endl;
                continue;
            }

            if (state.player1Board[x][y] == SHIP) {
                state.player1Shots[x][y] = HIT;
                std::cout << "Hit!" << std::endl;
            } else {
                state.player1Shots[x][y] = MISS;
                std::cout << "Miss!" << std::endl;
                state.currentPlayer = 1; // Переход хода к игроку 1
            }
        }

        // Проверка на победу
        int player1Ships = 0, player2Ships = 0;
        int player1ShipsOne = 0, player2ShipsTwo = 0;

        for (int i = 0; i < SIZE; ++i) {
            for (int j = 0; j < SIZE; ++j) {
                if (state.player1Shots[i][j] == HIT) {
                    player1Ships++;
                }
                if (state.player2Shots[i][j] == HIT) {
                    player2Ships++;
                }
            }
        }

        for (int i = 0; i < SIZE; ++i) {
            for (int j = 0; j < SIZE; ++j) {
                if (state.player1Board[i][j] == SHIP) {
                    player1ShipsOne++;
                }
                if (state.player2Board[i][j] == SHIP) {
                    player2ShipsTwo++;
                }
            }
        }

        if (player1Ships == player1ShipsOne) {
            std::cout << "Player 2 wins!" << std::endl;
            return;
        }
        if (player2Ships == player2ShipsTwo) {
            std::cout << "Player 1 wins!" << std::endl;
            return;
        }

        // Печать текущего статуса досок
        printBoard(state.player1Shots, "Player 1 Shots");
        printBoard(state.player2Shots, "Player 2 Shots");
    }
}

void startNewGame(GameState& state) {
    state.player1Board.resize(SIZE, std::vector<char>(SIZE, EMPTY));
    state.player2Board.resize(SIZE, std::vector<char>(SIZE, EMPTY));
    state.player1Shots.resize(SIZE, std::vector<char>(SIZE, EMPTY));
    state.player2Shots.resize(SIZE, std::vector<char>(SIZE, EMPTY));
    state.currentPlayer = 1;
    state.turnCount = 0;

    loadBoard("./newgame_player_1.cfg", state.player1Board);
    loadBoard("./newgame_player_2.cfg", state.player2Board);

    // Проверка количества кораблей
    if (!checkShipCounts(state.player1Board)) {
        std::cout << "Неверное количество кораблей Player 1. Проверьте конфигурацию." << std::endl;
        return;
    }
    if (!checkShipCounts(state.player2Board)) {
        std::cout << "Неверное количество кораблей Player 2. Проверьте конфигурацию." << std::endl;
        return;
    }

    std::cout << "Game started!" << std::endl;
    processTurn(state);
}

void continueGame(GameState& state) {
    state.player1Board.resize(SIZE, std::vector<char>(SIZE, EMPTY));
    state.player2Board.resize(SIZE, std::vector<char>(SIZE, EMPTY));
    state.player1Shots.resize(SIZE, std::vector<char>(SIZE, EMPTY));
    state.player2Shots.resize(SIZE, std::vector<char>(SIZE, EMPTY));

    std::ifstream file1("lastgame.cfg");

    loadBoard("./lastgame_player_1.cfg", state.player1Board);
    loadBoard("./lastgame_player_2.cfg", state.player2Board);
    loadBoard("./lastgame_shots_player_1.cfg", state.player1Shots);
    loadBoard("./lastgame_shots_player_2.cfg", state.player2Shots);

    if (file1.is_open()) {
        std::string turnCountStr, currentPlayerStr;
        std::getline(file1, turnCountStr);
        std::getline(file1, currentPlayerStr);
        state.turnCount = std::stoi(turnCountStr); // из строки в int
        state.currentPlayer = std::stoi(currentPlayerStr);

        file1.close();

        std::cout << "Game continued!" << std::endl;
        processTurn(state);
    } else {
        std::cout << "No saved game found. Returning to main menu." << std::endl;
    }
}

void cin_try(std::string& x)
{
    // Проверка ввода
    if (!(std::cin >> x)) {
        std::cout << "Invalid input. Please enter a number." << std::endl;
        std::cin.clear(); // Сброс состояния потока
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // Очистка буфера ввода
    }
}

bool authorization(const std::string login,const std::string pass, int player)
{
    std::string auth_file[] = {"./5.auth1.txt", "./5.auth2.txt"};
    
    int i {0};
    if(player == 1) i = 0;
    else if(player == 2) i = 1;
    else throw Except_log("Not correct player number");

    std::ifstream file(auth_file[i]);

    if (!file.is_open()) {
        std::cerr << "Could not open auth file." << std::endl;
        return false;
    }

    std::string correct_login, correct_pass;
    file >> correct_login;
    file >> correct_pass;

    file.close();

    std::cout << correct_login << " " << correct_pass << std::endl;
    
    if(login == "" && pass=="") throw Except_log_pass_empty("Login and password are empty");
    if(login == "") throw Except_log("Login is empty");
    if(pass == "") throw Except_pass("Password is empty");
    if(correct_login == login && correct_pass == pass) return true;
    else throw Except_log_pass("Login or password are not correct");
    
    return false;
}

int main() {
    GameState state;
    int choice;

    while (true) {
        
        // Авторизация
        bool player1 {false};
        bool player2 {false};
        while(true)
        {
            std::string login_one_player, pass_one_player;
            std::string login_two_player, pass_two_player;

            try
            {
                if(player1 == false)
                {
                    std::cout << "Игрок 1" << std::endl;
                    std::cout << "Введите логин: ";
                    cin_try(login_one_player);

                    std::cout << "Введите пароль: ";
                    cin_try(pass_one_player);

                    player1 = authorization(login_one_player, pass_one_player, 1);
                    if(player1 == true)
                    {
                        std::cout << "Доступ разрешен" << std::endl;
                    }
                }
                if(player2 == false)
                {
                    std::cout << "Игрок 2" << std::endl;
                    std::cout << "Введите логин: ";
                    cin_try(login_two_player);

                    std::cout << "Введите пароль: ";
                    cin_try(pass_two_player);

                    player2 = authorization(login_two_player, pass_two_player, 2);
                    if(player2 == true)
                    {
                        std::cout << "Доступ разрешен" << std::endl;
                    }
                }
            }
            catch(const Except_log& e)
            {
                std::cerr << e.ex_text() << '\n';
            }
            catch(const Except_pass& e)
            {
                std::cerr << e.ex_text() << '\n';
            }
            catch(const Except_log_pass_empty& e)
            {
                std::cerr << e.ex_text() << '\n';
            }
            catch(const Except_log_pass& e)
            {
                std::cerr << e.ex_text() << '\n';
            }
            catch(const std::exception& e)
            {
                std::cerr << e.what() << '\n';
            }

            if(player1 == true && player2 == true) break;
        }

        std::cout << "1. Start New Game" << std::endl;
        std::cout << "2. Continue Game" << std::endl;
        std::cout << "3. Exit" << std::endl;
        std::cout << "Choose an option: ";

        // Проверка ввода
        if (!(std::cin >> choice)) {
            std::cout << "Invalid input. Please enter a number." << std::endl;
            std::cin.clear(); // Сброс состояния потока
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // Очистка буфера ввода
            continue; // Возврат к началу цикла
        }

        std::cin.get(); // Получение символа перевода строки, если необходимо

        switch (choice) {
            case 1:
                startNewGame(state);
                break;
            case 2:
                continueGame(state);
                break;
            case 3:
                std::cout << "Exiting the game." << std::endl;
                return 0;
            default:
                std::cout << "Invalid choice. Please try again." << std::endl;
        }
    }

    return 0;
}